...
public abstract class NeoForgeLoadingOverlayMixin extends LoadingOverlay {
    ...
    public void rrls$render(GuiGraphicsExtractor context, int mouseX, int mouseY, float delta, CallbackInfo ci) {
        ...
        if (context instanceof DummyGuiGraphics || RrlsConfig.INSTANCE.skipForgeOverlay() || earlyLoadingScreenClosed) {
            ...
            ((OverlayExtender)this).rrls$setState(OverlayHelper.lookupState(minecraft.screen, false)); // Forge loading overlay is loading overlay :)
        }
    }
    ...
    public float rrls$changeAnimationSpeed(float instance) {
        if (!((OverlayExtender)this).rrls$getState().isRendering()) {
            return RrlsConfig.INSTANCE.animationSpeed();
        }
