...
public class OverlayHelper {
    public static State lookupState(Screen screen, boolean reloading) {
        if (!RrlsConfig.INSTANCE.hideType.get().canHide(reloading))
            return State.DEFAULT;

        if (reloading || RrlsConfig.INSTANCE.forceClose.get())
            return State.HIDE;
    ...
    public static boolean isRenderingState(Overlay overlay) {
        return overlay != null && ((OverlayExtender) overlay).rrls$getState().isRendering();
    }
