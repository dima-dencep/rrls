...
public abstract class LoadingOverlayMixin extends Overlay implements OverlayExtender {
    @Shadow
    @Final
    ...
    public void rrls$miniRender(GuiGraphics graphics) {
        ...
        switch (RrlsConfig.INSTANCE.type.get()) {
            case PROGRESS -> {
            ...
            case TEXT -> graphics.drawCenteredString(
                    minecraft.font, RrlsConfig.INSTANCE.reloadText.get(), i / 2, 70,
                    RrlsConfig.INSTANCE.rgbProgress.get() ? ThreadLocalRandom.current().nextInt(0, 0xFFFFFF) : -1
            );
        }
    }
    ...
    public boolean rrls$infinityLoading(Minecraft instance, Overlay overlay) {
        return RrlsConfig.INSTANCE.removeOverlayAtEnd.get();
    }
    ...
    public int rrls$rainbowProgress(int alpha, int red, int green, int blue, Operation<Integer> original) {
        if (RrlsConfig.INSTANCE.rgbProgress.get() && rrls$getState() != OverlayHelper.State.DEFAULT) {
    ...
    public float rrls$changeAnimationSpeed(float instance) {
        return RrlsConfig.INSTANCE.animationSpeed.get().floatValue();
    }
