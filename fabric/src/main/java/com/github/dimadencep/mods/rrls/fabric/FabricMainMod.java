    ...
    public void onInitializeClient() {
        ...
        HudRenderCallback.EVENT.register((matrixStack, tickDelta) -> {
            if (reloadHandler.getReload() != null && config.showInGame)
                DrawableHelper.drawCenteredTextWithShadow(matrixStack, this.client.textRenderer, I18n.translate(config.reloadText), this.client.getWindow().getScaledWidth() / 2, 70, config.rgbText ? getColor() : -1);
        });
        ...
        /*ScreenEvents.BEFORE_INIT.register((client, screen, scaledWidth, scaledHeight) -> {
            ScreenEvents.afterRender(screen).register((screen2, pose, mouseX, mouseY, delta) -> {
                if (reloadHandler.getReload() != null && config.showInGui)
                    DrawableHelper.drawCenteredTextWithShadow(pose, this.client.textRenderer, I18n.translate(config.reloadText), screen2.width / 2, 70, config.rgbText ? getColor() : -1);
            });
        });*/
